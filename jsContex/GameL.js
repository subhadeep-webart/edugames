...
class GameL extends Game {//console.log(" =" + );
	...
	picHit(x, y) {
		console.log("-picHit " + x + " " + y + "  " + cp.itf.nowPlaying + "  " + this.context.mode.singlePlayer);
		if (this.gameOver) {
			return;
		}
	...
	cleanPlayArea() {
		...
		if (GameContext.ui.gamePlayArea != undefined) {
			collection = GameContext.ui.gamePlayArea.children;
			console.log("collection  " + collection);//blueDot redDot
			for (let j = 0; j < collection.length; j++) {
