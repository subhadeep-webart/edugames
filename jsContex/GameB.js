...
class GameB extends Game{//console.log("GameB  = " + );
	...
	init(){console.log("GameB.init TOP"  );
		...
		GameContext.ui.bidButs.showButDisplay();
		GameContext.ui.bidButsstartBidClock();
		console.log("GameB.init BBBBOTTOM "  +  this.gameInPlay  + "  this.theRCAnswer= " + this.theRCAnswers);// 
	}

	cleanPlayArea() {
		console.log("******GameB cleanUpPlayArea  ");//GameContext.ui.ansBar
		super.cleanPlayArea();
		///hideJustAnswered();
		...
		if (GameContext.ui.ansBar != undefined) {
			GameContext.ui.ansBar.style = "display: none;";
		}
	...
	postAnswers() {//R1C1,R1C4,R2C3,R2C4,R3C2,R3C3,R3C4,1,1,1,4,2,3,2,4,3,2,3,3,3,4
		...
		for (let i = 0; i < GameContext.ui.bidButs.butMax   ; i++) {//this.ansDotLoc.length
	...
	aHit(theAns) {
		...
		console.log("***aHit theAns= " + theAns + " this.typeAnsText " + this.typeAnsText + " topBid= " + GameContext.ui.bidButs.topBid + " biddingInProgress= " + biddingInProgress +  " whoHasBid= "  + bidButs.whoHasBid)
		if (!gameInPlay) {
		...
		console.log("***-" + this.typeAnsText + "  targNbr= " + GameContext.ui.bidButs.topBid + "   hitCnt= " + this.hitCnt)
		if (this.typeAnsText != "T") {
			...
			console.log(" this.hitCnt= " + (this.hitCnt + 1) + " topBid=  " + GameContext.ui.bidButs.topBid) 

			if (++this.hitCnt == GameContext.ui.bidButs.topBid) {
...
*/
	...
	logPlay(reason) {
		console.log("GameB.logPlay reason= " + reason + "bidButs.topBid=  " + GameContext.ui.bidButs.topBid);
		let pf = "F"
		if (reason == "passed") {
			pf = "P";
		}
		...
		let buf = getSetRndData() + "," + cp.itf.nowPlaying + "," + this.timeLeft.toFixed(0) + "," + pf + "," + GameContext.ui.bidButs.topBid + "," + bufff;
	...
	checkPlay(reason) {
		console.log("*|*GameB.checkPlay reason= " + reason + " bidButs.topBid= " + GameContext.ui.bidButs.topBid + " butType= "  + this.butType)
		////this.playOn = false;//so players can't continue to hit
		const nbrHit = this.justAnswered.length;


		if (nbrHit != GameContext.ui.bidButs.topBid) {
			cp.itf.displayPtsThisPlay("failed", "Insufficient number of answers");
		...
		if (pf == "failed") {
			if (this.typeAnsText == "T") {
				GameContext.ui.ansBar.textContent = this.ansBarAllreadyAnswered;
			} else {
				this.logPlay(pf);
				this.unCheckCells();
				this.checkedRC = [];
			...
			results += "CORRECT!"
			this.markTheImages(cp.itf.nowPlaying);
			if(this.butType == 'T'){
				this.addToAnsBar(this.justAnswered);
			}
			if (this.typeAnsText == "T") {
				this.ansBarAllreadyAnswered = GameContext.ui.ansBar.textContent;
			}
        this.logPlay(pf);
			GameContext.ui.bidButs.reduceButtons(nbrHit, reason);
		...
		console.log("bottom of Game b checkPlay bidButs.remButs " + GameContext.ui.bidButs.remButs)
		this.justAnswered = []
		if (GameContext.ui.bidButs.remButs > 0) {
			this.resetPlay();
	...
	checkMarkIt(theRC) {
		console.log(" checkMarkIt  " + theRC + "  " + GameContext.ui.bidButs.whoHasBid);
		let imgSrc = "";
		if (onNet) {
			if (GameContext.ui.bidButs.whoHasBid == "Blue") {
	...
	procTypeG() {
		...
		GameContext.ui.bidButs.topBid = rightAnCount;
		GameContext.ui.bidButs.createButs(rightAnCount, "B", this.rows);//Includes fillBoxes


		//console.log("||||||||bidButs.topBid  = " + bidButs.topBid + "  " + theAnswers );
		for (let i = 0; i < GameContext.ui.bidButs.topBid; i++) {
	...
	addToJustAnswered(ans){console.log("addToJustAnswered ans= " + ans    )
		const x = GameContext.ui.ansBar.textContent;
		console.log("x== " + x   )
		GameContext.ui.ansBar.textContent = ans + "," + x;
	}
	...
	setJustAnswered() {
		...
		showJustAnswered();
		GameContext.ui.ansBar.textContent = "Your Answers: ";
	}
	setAnsBar() {
		console.log("Game B setAnsBar ")//For game type T
		showAnsBar();
		GameContext.ui.ansBar.textContent = "-";
	}
	...
	procAnsTypeT(x, y) {
		console.log("procAnsTyprT butTarg= " + GameContext.ui.bidButs.topBid         );
		if (biddingInProgress) {
		...
		if (++this.curAnsCnt == GameContext.ui.bidButs.topBid){
	...
	procData() {
		...
		console.log(" procData bottom bidButs.topBid= " + GameContext.ui.bidButs.topBid  + "  "   +  this.rows);
	} 
	...
	procTypeT(displayData) {
		...
		GameContext.ui.bidButs.createButs(this.rightAnsArray.length);
...
//                                             console.log("  = " + );	
	...
	typeMHit(theRC) {
		...
		console.log(GameContext.ui.bidButs.topBid +  "  ||this.checkedRC=	"  + this.checkedRC	);
		if (this.checkedRC.length == GameContext.ui.bidButs.topBid){
...
//        console.log("  " + )		
	...
	procTypeM() {
		...
		GameContext.ui.bidButstopBid = rightAnCount;
		GameContext.ui.bidButs.createButs(rightAnCount, "B");//Includes fillBoxes
	...
	setUpMultiImageTable(tblLayout, imageFileArr, selYesNo) {
		...
		for (let i = 0; i < rows; i++) {
			...
			for (let j = 0; j < cols; j++) {
				...
				GameContext.ui.bidButs.topBid = this.rightAnsArray.length
				console.log(cnt + " $$theImage$$  = " + imageFileArr[cnt]);

				console.log(GameContext.ui.bidButs.topBid + " ||*^^*|| " + this.rightAnsArray);
