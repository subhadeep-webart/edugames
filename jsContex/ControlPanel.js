...
class ControlPanel{            //          console.log(" = " + );
	constructor(context) {
		...
		console.log("cp constructor  top (this.context == null) = " + (this.context == null));
		console.log("cp constructor  top (this.context.ui.bidButs == null) = " + (this.context.ui.bidButs == null));
		this.utl;
		this.plu;
		this.itf;
		this.sp;
	...
	init(){
		...
		testIfOnNet();
		console.log("cp constructor  top (this.context == nundefined) = " + (this.context == undefined));
		this.utl = new Utl(this,this.context);
		////this.plu = new PlayerLineUp(this);
		this.itf = new Interface(this,this.context);
		this.itf.init();
		this.sets = new Sets();
		this.sets.init();
		...
		console.log("cp init (this.context == null) = " + (this.context == null));
		console.log("cp constructor  bottom of init (this.context.ui.bidButs == null) = " + (this.context.ui.bidButs == null));
		
	}

	startGame(aRound, sampleData) {//not here
		console.log("^^^cp.startGame onNet= " + onNet + " type= " + aRound);

		console.log("AA CP startGame this.context.ui.bidButs == null = " + (this.context.ui.bidButs == null));
		console.log("AA CP startGame GameContext.ui.bidButs == null = " + (GameContext.ui.bidButs == null));
		console.log("AA CP startGame GameContext == null = " + (GameContext == null));
		console.log("AA CP startGame this.context == null = " + (this.context == null));
		console.log("GameContext === this.context ?", GameContext === this.context);

		//this.context.ui.bidButs.stopBidClock();
		GameContext.ui.bidButs.stopBidClock();
		...
		switch (this.gameType) {
			case 'A':
				gameA = new GameA(aRound, this.context);
			...
			case 'B':
				this.itfType = "B"
				console.log("AA CP startGame this.context.ui.bidButs == null = " + (this.context.ui.bidButs == null));
				gameB = new GameB(aRound, this.context);
				console.log("BB CP startGame this.context.ui.bidButs == null = " + (this.context.ui.bidButs == null));
				this.theGameInPlay = gameB;
				gameB.init();
				break;
			case 'C':
				this.itfType = "B"
				gameC = new GameC(aRound, this.context);
				this.theGameInPlay = gameC;
				gameC.init();
				break;
			case 'D':
				this.itfType = "D"
				gameD = new GameD(aRound, this.context);
				this.theGameInPlay = gameD;
				///gameD.init();
				break;
			case 'E':
				this.itfType = "B"
				gameE = new GameE(aRound, this.context);
				this.theGameInPlay = gameE;
				gameE.init();
				break;
			case 'I':
				this.itfType = "I"
				gameI = new GameI(aRound, this.context);
				this.theGameInPlay = gameI;
				gameI.init();
				break;
			case 'L':
				this.itfType = "D"
				gameL = new GameL(aRound, this.context);
				this.theGameInPlay = gameL;
				//gameL.init();//We post a notice first then init
				break;
			case 'M':
				this.itfType = "B"
				gameM = new GameM(aRound, this.context);
				this.theGameInPlay = gameM;
				gameM.init();
				break;
			case 'N':
				this.itfType = "D"
				gameN = new GameN(aRound, this.context);
				this.theGameInPlay = gameN;
				///gameN.init();
				break;
			case 'O':
				this.itfType = "B"
				gameO = new GameO(aRound, this.context);
				this.theGameInPlay = gameO;
				gameO.init();
				break;
			case 'P':
				this.itfType = "B"
				gameP = new GameP(aRound, this.context);
				this.theGameInPlay = gameP;
				gameP.init();
				break
			case 'Q':
				this.itfType = "B"
				gameQ = new GameQ(aRound, this.context);
				this.theGameInPlay = gameQ;
				gameQ.init();
				break;
			case 'U':
				this.itfType = "B"
				gameU = new GameU(aRound, this.context);
				this.theGameInPlay = gameU;
				gameU.init();
				break;
			case 'X':
				this.itfType = "X"
				gameX = new GameX(aRound, this.context);//console.log(" =" + );
		...
		if(onNet){
			...
			fetch(url)
				...
				if(data != null){
					...
					setBeingPlayed = new Set(data, false);
	...
	*/
	...
	playNextRound(reason) {
		console.log("cp.playNextRound() top");
		if (setBeingPlayed == null) {
			console.log("No Set In play  " );
			return;
		}
		hideAnsTextBox(); 
		if (this.theGameInPlay != null) {
			this.theGameInPlay.cleanPlayArea();
		}
		setBeingPlayed.playNextRnd();
		console.log("cp.playNextRound() bottom");
