...
function checkXX(){console.log("script.check  " + cp.theGameInPlay );
	cp.theGameInPlay.checkPlay();
}

function setUpGameArea() {console.log("script.setUpGameArea  " );
	showAnsBut.style.visibility = "hidden"
	setUpPlayers();
}

function doXXX() {
	console.log("script. doXXX() ");
	bidButs.showButs();
	//bidButs.bidSeconds = 2;
	/*
	const arr = new Array();
	arr[0] = 24;
	arr[1] = "SkyBlue";
	...
	postNoticeCenterDisplay("18,Red,RED Has the PLAY");
	*/
...
function listRnds(){console.log("script.listRnds  "  );
	const theSetSerNbr = cp.setBeingPlayed;
	console.log("theSetSerNbr= " + theSetSerNbr );
	const theSetList = cp.setBeingPlayed.listRnds("XXX");
}
...
function getHelp(txt){console.log("script.getHelp  " + txt );
	///document.getElementById("helpDropDown").selectedIndex = 0//Reset the menu;
	if(txt == "roundNext"){
		if(cp.setBeingPlayed == null){
			alert("There is no Set in play at this time.");
			return;
		}else{
			const rndLtr  = cp.setBeingPlayed.getTypeOfNextRnd();
...
function setUpPlayers() {
	...
	bidButs.setBidTime(arr[3]);
	console.log("gp.bidButs.bidTime  " + bidButs.bidTime);
	plu.setPlayerNames(arr.shift());
	const setSerNbr = arr[1];
	console.log("gp setSerNbr= " + setSerNbr);//RndSelTes
	if (setSerNbr != "RndSelTest") {
		startTheGame(arr);//part of GP 
	}
...
console.log('script-This is the Bottom');
