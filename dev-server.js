...
const server = http.createServer((req, res) => {
  // req.url is whatever the client sent, and not every request comes from
  // our own pages -- a stale tab, a probe, or a page with a protocol-
  // relative href can send "//", which new URL() rejects with
  // ERR_INVALID_URL. Uncaught inside a request handler that throw takes
  // the whole server down, which looked like the port silently dying.
  let u;
  try {
    u = new URL(req.url, `http://localhost:${PORT}`);
  } catch {
    res.writeHead(400, { 'Content-Type': 'text/plain' });
    res.end('Bad request URL: ' + req.url);
...
// A dev server should never be killed by one bad request. Anything that
// escapes the handlers above is logged and the process keeps serving.
