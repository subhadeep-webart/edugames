...
class AlphaBar{
	...
	loadData(ansLstFileDotPath, rightAns) {
		...
		if (GameContext.ui.hideAlphaButtons) {
			GameContext.ui.hideAlphaButtons();
		}
	...
	checkAnswer(ans) {
		...
		GameContext.itf.setPlayerUp(this.playerAnswering);
		...
		if (this.rightAns == ans) {
			console.log("abp checkAnswer Success  " + "  " + GameContext.itf );
			GameContext.itf.displayPtsThisPlay("passed", "Got it Right");
			this.endGame(false)
			if (theGameInPlay != undefined) GameContext.itf.theGameInPlay.setGameOver();//For not set play
		} else {
			this.deleteAllAnswers();
			this.showAlphaButtons();
			console.log("abp checkAnswer Failure  ");
			GameContext.itf.displayPtsThisPlay("failed", "Sorry Wrong Answer")
	...
	endGame(gaveUp) {
		...
		//this.GameContext.itf.setGameOver(this.rightAns);
		if (gaveUp) {
			GameContext.itf.displayPtsThisPlay("failed", "You Gave Up and got -10 Points");
		}
	...
	getSelection(){console.log('getSelection theAns = ' + theAns +  "    " + GameContext.itf.nowPlaying);
		const el = document.getElementById("aBarDropDownMenu" + GameContext.itf.nowPlaying );
	...
	playerGivesUp() {
		...
		GameContext.itf.displayPtsThisPlay("failed", "You Gave Up");
		abp.endGame(true);
		//hideAlphaButtons();
		stopThePlayClock("End of Play");
	}
	...
	submitAnsxx(){console.log("abp checkAnswer " +  this.abp.getMenuSelection());
		...
		if(this.rightAns == theSelection){
			...
			GameContext.itf.displayPtsThisPlay ("passed");
			this.hideAllButs();
			console.log("aaa " );
			GameContext.itf.setGameOver();
			console.log("bbb " );
			stopPanelRemoval();
			stopThePlayClock("End of Play");
		}else{
			console.log("abp checkAnswer Failure  " );
			GameContext.itf.displayPtsThisPlay ("failed")
			//pauseButRemoval();
		}
		unPausePanelRemoval();
