...
function showAlphaButtons(side) {
    console.log("-showAnsBar " + side + alphaBarInsrtPtLeft.textContent + "   " + alphaBarInsrtPtRight.textContent);	
	if (side == undefined) {
...
function hideAlphaButtons(side) {
	console.log("hideAlphaButtons()  " + side + "   " +  alphaBarInsrtPtLeft.textContent + "   " + alphaBarInsrtPtRight.textContent);
	if (side == undefined) {
...
function showAlphaButtons() {
	console.log("script.showAlphaButtons()AA "  + insrtpt0Y);
...
function hideAlphaButtons(side) {
	console.log("script.hideAlphaButtons() "   + insrtpt0Y);
	if (side == undefined) {
...
/*
 ...
 function showAlphaButtons() {
	console.log("script.showAlphaButtons() " + ansBar);	
	ansBar.style.display = "block";
	ansBar.style.visibility = "visible"
}


function hideAlphaButtons() {
	console.log("script.hideAlphaButtons() " + ansBar);
...
function showJustAnswered() {console.log("script.showAlphaButtons() " + ansBar );		
	ansBar.style.display = "block";
	ansBar.style.visibility = "visible"	
}
...
function hideAnsTextBox() {
	console.log("script.hideAnsTextBox() "  + (ansBoxDiv == null)	);
...
function postNoticeCenterDisplay(arr) {//console.log("script.postNoticeCenterDisplay  " + arr);
	...
	const bgColor = arr[1]
	if (bgColor == "white") {
		centerDisplay.style.color = "black";
	} else { 
...
function blankNotice() {console.log("script.blankNotice  ");
	centerDisplay.style.fontSize = 16 + "px"
...
*/
function hide2nPlyr() {console.log("hide2nPlyr() " + singlePlayerMode)
	const xx = (secondPlayerBox == null);
	player1.style.display = "none";
	secondPlayerBox.style.display = "none";
}

function setUpPlayers() {
	console.log("setUpPlayers top"  + singlePlayerMode);//RndSelTest
	constructClasses();
	...
	console.log("setUpPlayers data  = " + data);//0;Peter;94566;12;10|1;Helen;94066;12;10,FromReg,PRA1_1842,20
...
function uniButClick(){console.log("script.uniButClic= " );
	GameContext.itf.ub.uniButClick();
}

function regPlayersXX(){ console.log("script.regPlayer  onNet= " + onNet);
	//doNetTest();
	cp = new ControlPanel();
	cp.init();
...
function explainNextRound(){console.log("script.explainNextRound " );
	GameContext.itf.explainNextRound();
}
...
function explainThisRound(){console.log("script.explainThisRound ");
	...
	if(cp == null){
		...
		GameContext.itf.explainThisRound();
	}
...
function playRndFromTBXX(){console.log("script.playRound ");
	if(cp == null){
		cp = new ControlPanel();
		cp.init();
	}
...
function startProgramXX(){console.log("script startProgram() " );
	cp = new ControlPanel();
	cp.init();
	help = new Help();
}
...
function runDotClock() {console.log("srunDotClock() " + dotSec);
	if (dotSec > 0) {
        dotSec--;
    }else if(!playOver){
...
function stopThePlayClock(reason){console.log("script.stopClock seconds = " + seconds + " orgSec= " + orgSec + " reason = " + reason  + "  playInProgress=   " + GameContext.itf.playInProgress);
	if (!GameContext.itf.playInProgress) return;
    timeWhenClockStoped = seconds;
	timeFac = seconds / orgSec;
	seconds = 0;
    clearInterval(interval);
	GameContext.itf.setTimeBonusPts(timeFac);
...
function regPlayersAndDisplaySelForm(){console.log("script regPlayersAndDisplaySelForm()  "  );
	GameContext.itf.reg2Players();
	GameContext.itf.removeInputForm();
...
function butHit(txt){console.log("script.butHit() " +  txt);
	...
	GameContext.itf.butHit(txt);//Redirect
}
