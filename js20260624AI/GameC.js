...
class GameC extends Game{//console.log(" =" + );
	...
	// Gospels;Mark;Matthew;,History;Acts;,Pauline Epistles;Corinthians 2;Corinthians 1;Romans;Ephesians;,Pastoral Epistles;Titus;,General Epistles;John 2;Jude;John 3;,Prophecy;Revelation;

	showAnswers() {console.log("GameC showAnswers()  " + this.gameDataArray);//
		let buf = "The Correct Categories:\n"

		for (let i = 0; i < (this.gameDataArray.length + 1); i++) {
			if (this.gameDataArray[i] == undefined) continue;
			const xx = this.gameDataArray[i].split(";");
			const n = xx.length;
			buf += xx[0] + " >> ";
			for (let j = 1; j < (n); j++) {
				if (xx[j] == undefined) continue;
				buf += xx[j];
				if (j < n - 2) {
					buf += " -- ";
				}
	...
	cleanPlayArea(){console.log("GameC cleanUpPlayArea  "  );//this.theAnsBar
		super.cleanPlayArea();
		if(this.tablDoc != undefined){
			this.tablDoc.remove();
		}
	}
...
freezeTheseItems(listOfItems,pNbr){console.log(pNbr + "  listOfAllPlacedItems " +  listOfItems )
    const array = listOfItems.split(",");//0= #0000FF,1= #FF0000
	for(let i = 0;i <array.length;i++){
		if(array[i].length > 1){
		this.itemsAllreadyPlaced.push(array[i]);
		const el = document.getElementById(array[i])
			if (el.nodeName == "INPUT" && el.style.color != "white"){
				el.style.background = plu.players[pNbr].color;
				console.log(pNbr + " ***** " + plu.players[pNbr].color + " ++  " + el.style.background)//1 ***** #8B0000;0 ***** #00008B
...
checkPlay(){console.log("************checkPlay  " +  "   targNbr= "  + bidButs.topBid  )//targNbr
	let reason = "";
	...
	const nbrButs = Number(bidButs.topBid)//For some reason this came back as a string			
	...
	if(pf == "failed"){
		...
		pf = "passed";
		this.freezeTheseItems(listOfAllPlacedItems, GameContext.itf.nowPlaying);//
		bidButs.reduceButtons(nbrButs);
	} 		
...
setUpPlayArea(){console.log("setUpPlayArea gameDataArray= "  + this.gameDataArray + "\n")
	 ...
	 GameContext.ui.gamePlayArea.innerHTML = GameContext.ui.gamePlayArea.innerHTML + htmlInput
      this.catMap = new Map();
	 ...
	 const mixedArray = cp.utl.mixUpArray(itemArray)

	 console.log("Game CCCCC AAA")
	 this.createButs(this.itemMax);//Includes fill boxes
	 console.log("Game CCCCC BBB")
