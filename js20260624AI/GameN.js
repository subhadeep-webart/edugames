...
class GameN extends Game{//console.log("GameN  = " + );
	...
	nextPlayer() {
		console.log("GameNnextPlayer()A nowPlaying= " + GameContext.itf.nowPlaying + "  " + this.slider.value);
	...
	setDisplay(aValue) {//console.log("setDisplay " +  aValue + "  playInProgress = " +  GameContext.itf.playInProgress );
		this.displayBox.innerHTML = aValue;
		/*
		if (GameContext.itf.playInProgress == false) {
			console.log("GameContext.itf.playInProgress " + GameContext.itf.playInProgress);
			GameContext.itf.postNotice("You need to click on a Press to Go First button.")
		} else {
	...
	singlePlayerHitMode() {
		...
		let ptAwd = (1 - ratio) * GameContext.itf.getPointsForThisRound();
		if (ptAwd < 0) ptAwd = 0;
	...
	evalTheResults() {
		...
		if(ans0 == ans1){
			...
			buf+= "It's a tie."
			GameContext.itf.setTieResults(this.pointsThisPlay,theRatio);
			return;
		}*/
		...
		GameContext.itf.displayResultsTypeD(winner, ptAwd, bufA);
		...
		//GameContext.itf.displayResultsTypeD(winner, ptAwd, buf);
...
//style="background:${greenBG};"
	...
	GameContext.itf.setQuestion(this.rndSerNbr + "--" + this.question);
