...
class GameD extends Game{
	...
	constructor(round, context) {
		...
		if (this.context.mode.singlePlayer) {
			...
			//this.twoPlayerSetUp()
			this.pickWhoGoesFirst();
			GameContext.itf.secondPlay = false;
		}
	...
	checkPlay(reason) {
		...
		this.regAns(0);
		//const thePlayerNbr = GameContext.itf.nowPlaying;
	...
	continueToSecondPlayer() {
		...
		//GameContext.itf.setPlayerUp(GameContext.itf.nowPlaying)
		let snd = null;
		//this.playerStartTimer()//
		GameContext.itf.setOtherPlayer();
		if (GameContext.itf.nowPlaying == 0) {
	...
	regAns(sliderLoc) {// player lets up on the mouse ="onchange" = regAns
		...
		console.log("|||||regAns isFirstPlay= " + this.isFirstPlay + "  nowPlaying =" + GameContext.itf.nowPlaying + " this.context.mode.singlePlayer= " + this.context.mode.singlePlayer)//Done on mouseUp of sliderDay. Starts second play
		...
		if (sliderLoc != 0) {
			...
			//this.logOfDisplayedAnswers[GameContext.itf.nowPlaying] = results;
			console.log("### results=   " + results + " nowPlaying=  " + GameContext.itf.nowPlaying);
		} 
		if (this.context.mode.singlePlayer) {
		...
		this.pAns[GameContext.itf.nowPlaying] = results;
		console.log("GameContext.itf.nowPlaying  = " + GameContext.itf.nowPlaying);
	...
	evalSinglePlayer(results) {	console.log("evalSinglePlayer()   " + this.theAnswer + "  " + this.pAnsX[0]);
		...
		GameContext.itf.displayResultsTypeD(0, points, playResults);
	...
	evalTheResults() {
		...
		//const thePoints = Number(GameContext.itf.getPoints());
		...
		//GameContext.itf.awardPoints("L",thePtAwd)
		...
		if (theRatioFac == 0) {//A tie
			...
			GameContext.itf.awardPoints(0, halfPts);
			GameContext.itf.awardPoints(1, halfPts);
		...
		this.placeAnswerArrow();
		GameContext.itf.displayResultsTypeD(theWinner, thePtAwd, buf);
...
<div style="text-align:center; font-sbr>ize:24px;"><input type="button" style="text-align:center; font-size:24px;" onclick="gameD.startGameD()" value="Start Round"></div>`
	...
	timesUp(){console.log("&&&&&.timesUp()  isFirstPlay=  " +  this.isFirstPlay);
		if(this.isFirstPlay){
			GameContext.itf.startSecondPlay();
			this.isFirstPlay = false;
		}else{
			GameContext.itf.displayPtsThisPlay();
		}
	}
	...
	onSelectionOfTheYear(sliderLoc){console.log("&&&&&&&&&&&onSelectionOfTheYear  " +  sliderLoc)
		...
		this.sliderYearLoc[GameContext.itf.nowPlaying] = sliderX;//This if for the arrows
	}
    ...
    twoPlayerSetUp() {
		...
		//this.thePlayer = plu.players[GameContext.itf.nowPlaying];
		this.thePlayer = this.context.players[GameContext.itf.nowPlaying];
