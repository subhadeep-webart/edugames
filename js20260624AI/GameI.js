...
class GameI extends Game{
	...
	*/
	...
	startGameI() {
		...
		GameContext.itf.setPoints(pointsForThisRound);
	...
	logPlay(anAns) {
        ...
        const player = cp.plu.getPlayerByNbr(GameContext.itf.getCurrentPlayerNbr());
...
//Cols=8,}A.AA.An.Ge.No.No.US.CiStMo.AL.csv Iwo Jima-Memorial,
	...
	procGameData(){console.log("GameI.procGameData TOP "  +  this.gameDataArray.length + "   " + this.gameDataArray);
		GameContext.itf.gameOver = false;//It may be true from last game
    ...
    removeAllFucusPanels() {
		...
		GameContext.itf.reduceThePts(100);
	}
	...
	removeAPanel() {console.log("removeAPanel "  + " this.displayType  " + this.displayType + "  "  + this.playIsOver);
		...
		if(this.typeSelection == "Author" && this.displayType != "BlockOut"){
			...
			if(butToRemove != null){
				...
				GameContext.itf.setGameOver();//We have run out of panels and no one has ventrued to try an answer
			}
		...
		GameContext.itf.reduceThePts(this.ptDecInc);
	}
