...
(function () {
  ...
  function paint() {
    ...
    /* Not a countdown right now ("--", "Time's up!", a bonus message).
       Between rounds the bar rests FULL, not empty: emptying it here made
       the next round's first tick animate 0% -> 100%, so the bar visibly
       filled up before it began draining. Snap, don't glide. Forget the
       scale so the next run re-learns it. */
    if (sec === null) {
      total = 0;
      setPct(100, true);
    ...
    /* A value above the high-water mark means a new countdown started
       (or the player changed the bid time), so re-scale to it. Re-scaling
       is a jump to a new frame of reference, not a tick, so it snaps too —
       otherwise the first tick of a round slides up from wherever the bar
       happened to be sitting. */
    var rescaled = sec > total;
    if (rescaled) { total = sec; }
  ...
  /* `snap` writes the width with the transition suppressed for one frame,
     for changes that are a jump rather than a countdown tick. */
  function setPct(pct, snap) {
    var w = pct.toFixed(2) + '%';
    ...
    fill.style.width = w;
    /* force a reflow so the no-transition width is committed before the
       transition goes back on, otherwise the two writes coalesce and it
       animates anyway. */
