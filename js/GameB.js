...
function tsdGameBNotice(title, body) {
...
class GameB extends Game{//console.log("GameB  = " + );
	...
	aHit(theAns) {
		...
		console.log("***aHit theAns= " + theAns + " this.typeAnsText " + this.typeAnsText + " topBid= " + bidButs.topBid + " biddingInProgress= " + biddingInProgress +  " whoHasBid= "  + bidButs.whoHasBid)
		if (!gameInPlay) {
		...
		console.log("***-" + this.typeAnsText + "  targNbr= " + bidButs.topBid + "   hitCnt= " + this.hitCnt)
		if (this.typeAnsText != "T") {
			...
			console.log(" this.hitCnt= " + (this.hitCnt + 1) + " topBid=  " + bidButs.topBid) 
...
*/
	...
	logPlay(reason) {
		console.log("GameB.logPlay reason= " + reason + "bidButs.topBid=  " + bidButs.topBid);
		let pf = "F"
		if (reason == "passed") {
			pf = "P";
		}
		...
		let buf = getSetRndData() + "," + cp.itf.nowPlaying + "," + this.timeLeft.toFixed(0) + "," + pf + "," + bidButs.topBid + "," + bufff;
	...
	checkPlay(reason) {
		console.log("*|*GameB.checkPlay reason= -" + reason + "- bidButs.topBid= " + bidButs.topBid + " butType= " + this.butType + "  " + gameInPlay)
		if (!cp.theGameInPlay) {
			return;
		}
		////this.playOn = false;//so players can't continue to hit
		if (reason === "timedOut" && bidButs.topBid === 0) {
			console.log("*|*GameB.checkPlay     reason == timedOut && bidButs.topBid === 0  "); 
            //alert("No bids were placed. Press Next Round");
			//postNoticeCenterDisplay("18,white,NO BIDS WERE PLACED PRESS NEXT ROUND!");
			return;
		}
		...
		if (pf == "failed") {
			if (this.typeAnsText == "T") {
				ansBar.textContent = this.ansBarAllreadyAnswered;
			} else {
				this.logPlay(pf);
				this.unCheckCells();
				this.checkedRC = [];
			...
			if (this.typeAnsText == "T") {
				this.ansBarAllreadyAnswered = ansBar.textContent;
			}
			this.logPlay(pf);
			bidButs.reduceButtons(nbrHit, reason);
		...
		/*
		if (singlePlayerMode && pf == "failed") {
			console.log("^^^^$$$$");
			cp.itf.displaySinglePlayerResultsOfPlay(pf, results);
		} else {
			cp.itf.displayPtsThisPlay(pf, results);
		}*/


		console.log("bottom of Game b checkPlay bidButs.remButs " + bidButs.remButs)
		this.justAnswered = []
	...
	procTypeG() {
		...
		bidButs.createButs(rightAnCount, "B", this.rows);//Includes fillBoxes
	...
	addToJustAnswered(ans){console.log("addToJustAnswered ans= " + ans    )
		const x = ansBar.textContent;
		console.log("x== " + x   )
		ansBar.textContent = ans + "," + x;
	}
	...
	setJustAnswered() {
		...
		showJustAnswered();
		ansBar.textContent = "Your Answers: ";
	}
	setAnsBar() {
		console.log("Game B setAnsBar ")//For game type T
		showAnsBar();
	...
	procAnsTypeT(x, y) {
		console.log("procAnsTyprT butTarg= " + bidButs.topBid         );
		if (biddingInProgress) {
	...
	procData() {
				...
				fetch(filePath)
					...
					.then(data => {
						if (data != null) {
							...
							tsdGameBNotice("Download Failed", "Something went wrong and the text could not be downloaded.");
							return;
						}
					});
				console.log("***** CCC")
		...
		console.log(" procData bottom bidButs.topBid= " + bidButs.topBid  + "  "   +  this.rows);
	} 
	...
	typeMHit(theRC) {
		...
		for (let i = 0;i < this.checkedRC.length; i++){
			...
			if(theRC == this.checkedRC[i]){
				tsdGameBNotice("Already Checked", "You have already checked this one.")
				return;
			}
		}
...
//        console.log("  " + )		
	...
	procTypeM() {
		...
		bidButs.topBid = rightAnCount;
		bidButs.createButs(rightAnCount, "B");//Includes fillBoxes
...
//                                             console.log("  = " + );
	...
	checkMarkItXX(theRC) {
		console.log(" checkMarkIt  " + theRC + "  " + bidButs.whoHasBid);
		let xPt = 0; let yPt = 0
		let buf = ""
		if (onNet) {
