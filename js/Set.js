...
/* Round comments and instructions used window.alert(), which renders the
   browser's own grey chrome over the themed game. Route them through the
   shared dialog in js-ui/tsd-modal.js instead, falling back to alert() if
   that layer is absent.

   NOTE: only for messages whose caller does NOT depend on alert()'s
   BLOCKING behaviour. TSDModal.alert() returns immediately, so any call
   site that has work to run "after the player clicks OK" must sequence it
   through onConfirm -- see the "Ins" case in playNextRnd(). */
function tsdSetNotice(title, body) {
...
class Set{
	constructor (input,isFmSubset,from){console.log("Set.NEW  " +  input + "  isFmSubset= " + isFmSubset + "  from= " + from);
		this.input = input;
        this.rndcount = 0;
		this.setMap;
		this.rndArr = [];
	...
	playNextRnd(from) {
		console.log("Set.playNextRnd() this.rndArr=  " + this.rndcount + " -- " + this.rndArr + " from= " + from);
		...
		switch (type) {
			case "Rnd":
				if (onNet == true) {
					...
					////fetch('/edugames.com/cgi-bin/GetRo
					fetch(url, {
					})
					...
					//Offline: SelectionPanel.getRoundFmRnd() dispatches on the
					//game type letter to the local RoundB/C/D/... data classes
					//and returns a Round. Same handling as the fetch above.
					const round = cp.sp.getRoundFmRnd(rnd);
					if (round == null) {
						console.log("Set.playNextRnd offline: no data for " + serNbr);
						this.playNextRnd("offline-miss");
						return;
					}
					cp.itf.cleanPlayArea();
					theQuestion = round.getTheQuestion();
			...
			case "Ins":
				...
				/* The native alert() BLOCKS, so the playNextRnd() below only
				   ran once the player clicked OK. TSDModal.alert() does not
				   block, so advancing on the next line would move the round
				   on behind the open dialog. Sequence it in onConfirm to
				   preserve the original order: show, wait, then advance. */
				if (window.TSDModal && typeof TSDModal.alert === "function") {
					const self = this;
					TSDModal.alert({
						title:   "Instruction",
						message: tempArr[1],
						icon:    "help",
						onConfirm: function () { self.playNextRnd("ins"); }
					});
				} else {
					alert(tempArr[1]);
					this.playNextRnd("ins");
				}
				break;
			case "Wpg":
...
Rnd;AA.Qen00054;1;11.00;100;0;Wars/US/American Revolution;Connect the Revolutionary Battles.;
	...
	*/
	...
	async breakOutData(input) {
		...
		const processItem = async (item) => {
			...
			if (type === "Set") {
				...
				console.log(`Processing nested set ${subSetSerNbr}, url=${url}, onNet=${onNet}`);
				...
				try {
					...
					if (onNet) {
						...
						//Offline: read the subset from Sets.js, the same source
						//ControlPanel.createSet() uses when onNet is false.
						text = cp.sets.getData(subSetSerNbr);
						if (text == null) throw new Error(`subset ${subSetSerNbr} not in Sets.js`);
					}
					text = deComma(text);
	...
	getRoundFmRnd(rnd) {
		...
		if (onNet == true) {
			const url = '/cgi-bin/GetRoundsTSD.pl?' + rndSerNbr;
	...
	async getNextRndZZZ(from) {//function //getNextRnd(from)
		...
		try {
			...
			if (type === "Set") {
				...
				if (onNet) {
					const url = '/cgi-bin/GetASetTSD.pl?' + aSerNbr;
				...
				tsdSetNotice("Comment", rndOrSetArr[7]);
			} else if (type === "Wpg") {
				window.open(this.rndArr[6], '_blank');
			}
	...
	async getRndsFromSubSet(aSet) {
        ...
        const lstOfRnds = [];
		const url = '/cgi-bin/GetASetTSD.pl?' + theSetSerNbr;
		console.log("url = " + url);
		...
		try {
			...
			if (data != null) {
				...
				tsdSetNotice("Download Failed", "Something went wrong and the Set could not be downloaded.");
			}
		} catch (error) {
				...
				subsetPromises.push(
					fetch(url)
						...
						.catch(err => {
							...
							tsdSetNotice("Subset Not Loaded", "Subset could not be loaded: " + subSetSerNbr);
						})
				);
