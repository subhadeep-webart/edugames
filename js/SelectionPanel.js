    ...
    getRoundFmRnd(rnd){console.log("sp.getRoundFmRnd  " + " onNet= " + onNet  + "  rnd= " + rnd);//theRndMenu
      ...
      let roundData ="";
       console.log("||sp.gameType=  "+ gameType)
        ...
        //The CGI response carries one extra leading field that the local
        //Round*.js data does not. Round.breakOutData() shifts once and then
        //reads arr[1], so without this the serial number is off by one and
        //gameType comes out of the author field ("EdUGames tm" -> "G").
        //Prepend a placeholder so the offline string matches the online shape.
