...
`;
	...
	changePlayers(text){console.log("Game  changePlayers = "  );//console.log(" =" + );
		...
		const msg = ((text != null).length > 0)
			? (text + " and the next player is " + nextPlayer.name)
			: ("The next player is " + nextPlayer.name);

		/* The name field below used to update AFTER the player dismissed the
		   native alert(), because alert() blocks. TSDModal.alert() does not,
		   so run that update from onConfirm to keep the same order --
		   otherwise the next player's name appears behind the open dialog. */
		const self = this;
		const applyNextPlayer = function () {
			self.tfPlayerName.value = nextPlayer.name;
			self.tfPlayerName.style.backgroundColor = nextPlayer.altColor;
		};
