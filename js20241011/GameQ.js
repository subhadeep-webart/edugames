...
class GameQ extends Game{
	...
	procAnsTypeRM(anAnswer){console.log("||||||||procAnsTypeRM "  + anAnswer + "  " + (this.nbrHit + 1 ) + "   " + cp.itf.nbrButSelected);
	...
	checkPlay(){console.log("##########GameQ.checkPlay TOP this.rw = " +  this.rw + "  nbrButSelected= " + cp.itf.nbrButSelected);
		stopTheClock();
		this.ansPending = false;
		let pf = "passed";
		let results ="";
		const nbr = this.rw.length;
		if(nbr < cp.itf.nbrButSelected){
