...
const sampleRound = ",AA.Ien00005,EdUGames tm,,9,,,,Te,,}P.AA.Pi.Ge.No.No.US.SD.Mo.MtRushmore-1.PI.gif,aPRA1_70 iPRA1_15 oPDF1_5 zPRA1_15,,,,,US/Monuments,What is this American Monument?,,DisplayType=Grid TypeSelection=Player Rows=3 Cols=3,}L.AA.An.Ge.No.No.US.CiStMo.AL.csv Rushmore-Mt.,}P.AA.Pi.Ge.No.No.US.SD.Mo.MtRushmore-1.PI.gif,{H-Northern State;-;4 Presidents;In Stone,,,,,,,";
...
class Game{
	...
	breakOutRound(){
		...
		console.log("Game.this.gameData= "  +  this.gameData[1]);
...
class GameI extends Game{
	...
	procGameData(){
		console.log("GameI.procRound TOP "  +  this.gameData);
		const displayType = this.parmMap.get('DisplayType');
		const typeSelection = this.parmMap.get('TypeSelection');
		const imageAndAnswer = this.gameData[4];
		const gameDataArray = this.gameData.split(',');
		const temp = gameDataArray[0].split(' ');
		this.ansLstFile = temp[0];
		this.ans =  temp[1];
        console.log("GameI.procGameData this.ansLstFile "  + this.ansLstFile + " this.ans " + this.ans);
	}

	loadAlphaBar(ansLstFile){
        console.log("GameI. loadAlphaBar top"   );
		this.abp =	new AlphaBar(sampleAnsLst,this.ans,this);
		this.abp.init();
	...
	insertImage(){////   images/MtRushmore-1.PI.gif
		...
		//const but = '<input type="button" name="overLay" id="butOverlay" width="496" height="288" value="XXXXX">';
		//imageOverLayPt.innerHTML = imageOverLayPt.innerHTML + but;

	}

	addGridButton(){
		const butStyle = `<style> 
    #butR0C0{
    top:15%;
    left:15%;
...
`;
		...
		for(let i = 0;i< rows;i++){
			for(let j= 0;j< cols;j++){//this.rows
				...
				//bufBut+=`<button class='butX' style="width:${width}px;"  style="height:${height}px;"  name='XXX${butID}' id='${butID}' >-${width}}|${height}-</button>\n`;
		...
		//imageOverLayPt.innerHTML = imageOverLayPt.innerHTML + bufBut;



		const testBut = document.getElementById('but0|0');
        //console.log("GameI. testBut= " +  testBut.name);
	...
	createGrid(){
		console.log("GameI.procRound TOP rows = "  +  this.rows + '  cols= ' + this.cols);
...
function test(){
	...
	const rows = 5, cols=4,imageWidth = 488, imageHeight = 288;
		const width             = Math.round(imageWidth/cols);
		const height            = Math.round(imageWidth/rows);
		const percentWidth      = 100/(cols);
		const percentHeight     = 100/(rows);
		const halfWidth        = width/2;
		const halfHeight        = height/2;
		...
		for(let i = 0;i< rows;i++){
			for(let j= 0;j< cols;j++){//this.rows
				...
				bufBut+=`<button class='butX' style="height:${height}px;width:${width}px"  id='${butID}' >${butID}</button>\n`;
