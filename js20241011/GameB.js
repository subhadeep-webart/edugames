...
class GameB extends Game{//console.log("GameB  = " + );
	...
	cleanPlayArea(){console.log("******GameB cleanUpPlayArea  "  );//this.theAnsBar
		...
		if(this.theAnsBar != undefined)this.theAnsBar.remove()
	...
	getData(){console.log("$$$GameB getData  "  );
		...
		if(this.typeGame == "A"){//answerButton file that has to be retrieved
		...
		console.log(" getData bottom cp.itf.nbrButSelected= " + cp.itf.nbrButSelected);
	}// 



	checkPlay(reason){console.log("*|*GameB.checkPlay reason= " + reason + " justAnswered= " + this.justAnswered + "  this.playOn = " + this.playOn)
		this.playOn = false;//so players can't continue to hit
		const nbrHit = this.justAnswered.length

		if(nbrHit != cp.itf.nbrButSelected){
	...
	createGridMap(imageWidth,imageHeight,boxWidth,boxHeight,nbrRows,nbrCols){//console.log("createGridMap"  );
		...
		//console.log(rc  + " = " + this.gridMap.get(rc) );
			}
		}

		//console.log(" R2C2= " + this.gridMap.get("R2C2") +   "  R2C1 " +   this.gridMap.get("R2C1"));
...
//                                             console.log("  = " + );
	...
	procTypeT(displayData){//Transparent buttons//		this.rightAnsArray = []
		...
		cp.itf.setTheNbrOfbuts(this.rightAnsArray.length);
        console.log(" cp.itf.nbrButSelected = " + cp.itf.nbrButSelected);
	...
	addToAnsBar(ans){console.log("addToAnsBar  " + ans + "  this.setAnsBar()= " + this.setAnsBar())
		if(this.theAnsBar == undefined)this.setAnsBar();

		this.theAnsBar.textContent = ans + "," + this.theAnsBar.textContent;
	}
	...
	getTexAnsFmXY(x,y){console.log("getTexAnsFmXY "  + x + "  " + y )
		...
		for(let i = 0;i< this.recArray.length;i++){
			...
			if((x > Number(recParts[0]) && x < (Number(recParts[0]) + Number(recParts[2]))) &&  (y > Number(recParts[1]) && y < (Number(recParts[1]) + Number(recParts[3])))){
				...
				if(++this.curAnsCnt == cp.itf.nbrButSelected){
					stopTheClock();
					this.checkPlay();
				}
			}
	...
	procAnsTypeT(x,y){console.log("procAnsTyprT x= " +  x + ' y= ' + y  + " this.playOn=  " + cp.itf.playInProgress);
        if(!cp.itf.playInProgress){
			alert("You need to select a number Button first.")
			return;
		}
		this.getTexAnsFmXY(x,y);
		...
		let buf = "<table border='1'>"
			for (let i = 0;i < rows; i++){
				...
				for (let j = 0;j < cols; j++){
					...
					cp.itf.nbrButSelected = this.rightAnsArray.length
                   console.log(cnt + " $$theImage$$  = " + imageFileArr[cnt]);

					console.log(cp.itf.nbrButSelected + " ||*^^*|| " + this.rightAnsArray);
...
//                                             console.log("  = " + );	
	...
	procTypeM(){console.log("procTypeM() length= " + this.gameDataArray.length);
	...
	typeMHit(theRC){console.log("typeMHit  = " + theRC  );	//this.playerArray
	  	if(!cp.itf.playInProgress){
		...
		console.log( cp.itf.nbrButSelected +  "  ||this.checkedRC=	"  + this.checkedRC	);
		if(this.checkedRC.length == cp.itf.nbrButSelected){
...
//                                             console.log("  = " + );	
	...
	aHit(theAns){console.log("-aHit theAns= " + theAns  + " cp.itf.playInProgress = " + cp.itf.playInProgress)
		if(!cp.itf.playInProgress){
			//cp.itf.postNotice("You need to select a button first.")
			alert("You need to select a button first.");
			return;
		}
		for (let i = 0;i < this.justAnswered.length; i++){
			if(theAns == this.justAnswered[i]){
				alert("You have just answered this one.");
				return;
			}
		}
		if(this.isAllreadyAnswered(theAns)){
			alert("This has allready been answered.")
			return;
		}

		console.log("-" + this.typeGame + "   " +  cp.itf.nbrButSelected  +  "    " + this.hitCnt)
		if(this.typeGame != "T"){
			if(this.typeGame == "G"){
			...
			if(++this.hitCnt == cp.itf.nbrButSelected){
	...
	startPlay(trgHitNbr){console.log("GameB  startPlay " + trgHitNbr )
		cp.itf.nbrButSelected = trgHitNbr;
		this.hitCnt = 0;
		this.justAnswered = [];
		this.playOn = true;
	}
...
//                                             console.log("  = " + );
	procTypeG(){console.log("++++++++++++ procTypeG "  );
		...
		cp.itf.nbrButSelected = rightAnCount;
		cp.itf.setTheNbrOfbuts(rightAnCount,"B");

        //console.log("||||||||cp.itf.nbrButSelected  = " + cp.itf.nbrButSelected + "  " + theAnswers );
		for (let i = 0;i < cp.itf.nbrButSelected; i++){
