...
class GameL extends Game{//console.log(" =" + );
	...
	startPtFacInc(){console.log("gameL startPtFacInc " );
		setUpTheIncreasingPtFac(1.0,-0.05,2);//this makes it increase
	 }

	picHit(x,y){console.log("*******picHit "  + x + "   " + y  + " playInProgress= " + cp.itf.playInProgress + " this.playCount = " + this.playCount + " sidePlayin= " + cp.itf.nowPlaying);
	...
	checkPlay(from){console.log("|||checkPlay this.locArr= " +  this.locArr + "  map.get(Loc)= " + this.map.get("Loc"));//        console.log(" "  + )
		...
		let buf = "The answer is indicated by the black cross. The closests player was "
        console.log("loc-"  + loc + "-")
		const result =[];
		...
		const postedPts = cp.itf.getPoints();
