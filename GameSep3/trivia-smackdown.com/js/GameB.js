...
class GameB extends Game{//console.log("GameB  = " + );
	...
	addToQBar(txt){console.log("addToQuestionBar " +  txt)//Clean out wrong answers
		const x = this.qBox.textContent;
        console.log(" x= " + x)
		this.qBox.textContent =  x + ", " +  txt;
        console.log(" this.qBox.textContent= " + this.qBox.textContent)
	...
	procAnsTypeT(x,y){console.log("procAnsTyprT butTarg= " + cp.itf.targNbr );
		...
		if(++this.curAnsCnt == cp.itf.targNbr){
	...
	getData(){console.log("$$$GameB getData  "  );
		...
		//console.log(" getData bottom cp.itf.targNbr= " + cp.itf.targNbr);
	} 
		...
		let buf = "<table border='1'>"
			for (let i = 0;i < rows; i++){
				...
				for (let j = 0;j < cols; j++){
					...
					cp.itf.targNbr = this.rightAnsArray.length
                   console.log(cnt + " $$theImage$$  = " + imageFileArr[cnt]);

					console.log(cp.itf.targNbr + " ||*^^*|| " + this.rightAnsArray);
...
//                                             console.log("  = " + );	
	...
	typeMHit(theRC){console.log("typeMHit  = " + theRC  );	//this.playerArray
		...
		console.log( cp.itf.targNbr +  "  ||this.checkedRC=	"  + this.checkedRC	);
		if(this.checkedRC.length == cp.itf.targNbr){
...
//                                             console.log("  = " + );	
	...
	aHit(theAns){console.log("-aHit theAns= " + theAns  + " this.typeGame " + this.typeGame)
		if(!cp.itf.playInProgress){
			//cp.itf.postNotice("You need to select a button first.")
		...
		console.log("-" + this.typeGame + "  targNbr= " +  cp.itf.targNbr  +  "   hitCnt= " + this.hitCnt)
		if(this.typeGame != "T"){
			if(this.typeGame == "G"){
...
//                                             console.log("  = " + );
	procTypeG(){console.log("++++++++++++ procTypeG "  );
		...
		cp.itf.targNbr = rightAnCount;
		cp.itf.setTheNbrOfbuts(rightAnCount,"B");

        //console.log("||||||||cp.itf.targNbr  = " + cp.itf.targNbr + "  " + theAnswers );
		for (let i = 0;i < cp.itf.targNbr; i++){
