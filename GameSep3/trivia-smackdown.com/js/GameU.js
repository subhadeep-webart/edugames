...
//Complete the Bible sayings.                    ,^AA.DCL.E,Rows=7 Cols=3 FntSize=14 FntColor=blue BkGndColor=lightGray LnCnt=1 SingleScreen=No Sort=Yes
	...
	checkPlay(){console.log("####checkPlay = " + " targNbr= " + cp.itf.targNbr + "  " + this.butHitArrArr + " this.nbrHit= " + this.nbrHit);
		//stopTheClock();
		let pf = "passed";
		let results ="";
		if(this.nbrHit != cp.itf.targNbr){
	...
	isTrgNbrRechedXX(){console.log("|||isTrgNbrReched "  +  (this.nbrHit + 1 ) + "   " + cp.itf.targNbr);
		this.ansPending = false;
		if(++this.nbrHit == cp.itf.targNbr){
			cp.itf.timeFactor = stopTheClock();
			return true;
