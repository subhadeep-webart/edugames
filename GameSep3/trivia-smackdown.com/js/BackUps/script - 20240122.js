...
function startGame(){//AA.Men00021
	...
	plu.registerPlayer("Pete");
	plu.registerPlayer("Mary");
	plu.registerPlayer("Paul")
...
function testA(){	//}P.AA.Pi.Th.Tr.Na.SailBoat.KGDB.gif
	const anArray = getImageFile("}P.AA.Pi.Th.Tr.Na.SailBoat.KGDB.gif","theImageID");
	console.log("testA()anArray- " + anArray );
...
class PlayerLineUp{
	...
	constructor (cp,utl){
		this.players = [];
		this.color = ["Blue","Red","Green","Pink"];
		this.pNbr = 0;
	...
	registerPlayer(name){
		const aPlayer = new Player(name,this.color[this.pNbr++]);
...
class GameN extends Game{//console.log("GameN  = " + );
	...
	init(){
		...
		//this.setSliderEventHandler();
		//this.procSym();
		//this.insrtImage();
		console.log("GameN.init BOTTOM "  +  this.gameInPlay);//  
	}
...
//####
	...
	evalAnswers(anAnswer ){
		console.log("GameN evallAnswer()" + anAnswer )
	...
	setValues(){//,Answer=500 Type=Integer LowBracket=200 HiBracket=1200,
		...
		const slider = document.getElementById("slider");//min="1" max="100"
		slider.min = this.loNbr;
		slider.max = this.hiNbr;
	...
	insrtBlueArrowXX(){///Pi/Th/Sy/To/Ar/BlueUpArrow/BlueUpArrow.AA.jpg' id='undefined' />
		const img = getImageFile("}P.AA.Pi.Th.Sy.To.Ar.BlueUpArrow.AA.jpg","blueArrow","class='arrow'  draggable='true'  ")
		const insrtPt = document.getElementById("arrowInsrtPt");
		insrtPt.innerHTML = insrtPt.innerHTML + img[0];
		const theArrow = document.getElementById("blueArrow");
		const theDisplay = document.getElementById("displayBox");

		theArrow.addEventListener("dragstart", function(event){
			console.log("dragstart");
			event.dataTransfer.setData("text/plain", "This text may be dragged")
		})
		theArrow.addEventListener("dragend", function(event){
			console.log("dragend   " + event.x);
			theArrow.style.left =((event.x -17) +"px");
			event.dataTransfer.setData("text/plain", "This text may be dragged")
		})
	...
	setSliderEventHandlerXX(){
		console.log(" setSliderEventHandler" );
		const slider = document.getElementById("slider");
		console.log("slider "  + slider.value);
		
		slider.addEventListener("onoutput", function(event){
			console.log(" onoutput "  + slider.value );
			gameN.nextPlayer(slider.value);
		})
...
function dragElement(elmnt) {
  ...
  elmnt.style.left = (elmnt.offsetLeft - pos1) + "px";
    ...
    document.onmouseup = onMouseUp(event.x);
...
//<img src="../../HTDocs/public_html/edugames.com/DataBase/A65AA65A/ResLibry/Pi/Th/Sy/To/Ar/BlueUpArrow/BlueUpArrow.AA.jpg" width="32" height="32" alt="" title="" />
