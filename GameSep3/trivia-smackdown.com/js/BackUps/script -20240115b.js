...
function getImageFile(fileData){
...
//              }P.AA.Pi.Th.Tr.Na.SailBoat.KGDB.gif,{A-4red 009118038066 180,{A-4red 373118038066 180,{R-4red 004054057057,{W-4red 361055057057,
	startGame(){console.log("GameA. "  + this.gameDataArray)
		...
		const len = this.gameDataArray.length;//this.imageName = 
		for(let i = 0;i <len;i++){
			const object = this.gameDataArray[i].substring(1,7);
			const type = object.charAt(0)
			const size = object.charAt(2)
			const color = object.substring(3,7);

			const sizeData = this.gameDataArray[i].substring(8,20);
			const orientation = this.gameDataArray[i].substring(21,24);//console.log("GameA  = " + );

			console.log("type =" + type);//gameAImageInsrtPt
	...
	insrtCanvas(){console.log("||||insrtCanvas =" );
		...
		const canvas = "<canvas id='myCanvas' color='green' width='600' height='600'></canvas>";
		canvasInsertPt.innerHTML = canvasInsertPt.innerHTML + canvas;
		const myCanvas = document.getElementById("myCanvas");
		const ctx = myCanvas.getContext("2d");
		ctx.fillRect(100, 200, 50,50,width,);

		let theImage = new Image(path,50,50,width,height);

		this.createImage(ctx,path,50,50,width,height);
