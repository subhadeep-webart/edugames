...
/*
	...
	fillTheBoxes(inputArray){ console.log("GameO.fillTheBoxes "  + inputArray);
		const len = inputArray.length;	
		let buf =  "`<table border='3'  width='100%'><tr>><td width='50%' background-color='green'><div class='container' id='left'>left`";
	    const el = document.getElementById("gameInsrtPt");
		for(let i = 0;i <len;i++){
		...
		const containers = document.querySelectorAll('.container')
							//console.log("GameO.containers= "  + containers.length);
		containers.forEach(container =>{
							//console.log("GameO.container= "  + container.id);	//	 	
			container.addEventListener('dragover',e =>{
				e.preventDefault()
				const afterElement = this.getDragAfterElement(container, e.clientY)
				console.log("afterElementID=" + afterElement.id)
	...
	getDragAfterElement(container, y) {
	   console.log(container.id +   " y= " + y)
	   const draggableElements = [...container.querySelectorAll('draggable:not(.dragging)')]
	   //const draggableElements = [...container.querySelectorAll('.draggable')] 
	   console.log("draggableElements.len " + draggableElements.length)
	   console.log("draggableElementsclass= " + draggableElements.class)   
		return draggableElements.reduce((closest,child) =>{
			const box = child.getBoundingClientRect()
			const offset = y - box.top - box.height / 2 
	...
	draggables = document.querySelectorAll('.draggable')//we redo this now that we have draggables inserted
		console.log("draggables.length= " + draggables.length);
		draggables.forEach(draggable => {
		draggable.addEventListener('dragstart',()=>{
		console.log("XXXXX" )
