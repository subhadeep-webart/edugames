...
class GameC extends Game{//console.log(" =" + );
	...
	startGame(game){
      console.log("startGame XXX"  );//gameButInsrtPt
      ...
      const catRows = 3;
      const  catCols = 2;
       //this.inputArray = this.gameDataArray
      ...
      let bufItems = ""
      for(let i = 0;i <itemArray.length;i++){//background-color='#ffff00'
        bufItems += "<input type='button' id='" + itemArray[i] +"' class='draggableC' draggable='true' value='" + itemArray[i] + "'>\n";       
      } 
      ...
      for(let i = 0;i <catRows;i++){
         ...
         for(let j = 0;j <catCols;j++){
                 const butID = "cat" + i + "X" + j;//draggable='true'  class='draggable' 
                 const catName =        catArray[n++]
                 buf+= "<td class='containerC' id='" + catName + "'><p  style='text-align:center'  class='catName' id='" + catName + "' > " + catName + "</p></td>\n";
         } 
         buf+="</tr>"
     }  
     buf+= "</table>";
   ...
   containers.forEach(container =>{                             
	...
	killAllItems(listOfAllDropedIems){
		console.log("listOfAllDropedIems " +  listOfAllDropedIems)    
		const array = listOfAllDropedIems.split(",");
	 ...
	 returnItemToLeft(listOfAllDropedIems){
		 console.log("returnItemToLeft  " + listOfAllDropedIems );
		 const array =  listOfAllDropedIems.split(",");
	 ...
	 check(){
		...
		const catsWithItems = [] 
		let listOfAllDropedIems =  ""//so we can either move them back or freeze them
		...
		for(let i = 0;i <containers.length;i++){
			...
			if(el != null){
			  ...
			  if(arrayX.length > 1){
				...
				listOfAllDropedIems+=  arrayX.slice(1) +  "," ;      
			  }
			}
	   }
	   ...
	   if(gotOneWrong){
		  console.log("Failed "  )
		  this.returnItemToLeft(listOfAllDropedIems);    
	   }else{
		 console.log("Success "  )
		 this.killAllItems(listOfAllDropedIems)
	   }      
	 }
