...
class Game{
			...
			for (let i = 0;i<this.rows;i++){
				for (let j = 0;j<this.cols;j++){
					this.insrtIdArray.push(i + ':' + j);
										...
										console.log("this.insrtIdArray = " + this.insrtIdArray);
		}
	...
	resetPage(){
		...
		document.getElementById('ptAwd').value = "0";
	}
	...
	awdPoints(amt){
		...
		document.getElementById('ptAwd').value = parseInt(n, 10) + amt;
...
class BoxGame extends Game{//console.log(" =" + ); console.log(" TOP"  );
	...
	loadButtonsIntoGrid(gameType){//BoxGane
		...
		for (let i = 0;i<this.nbrOfButs;i++){//Place the button with the same id as the insrtPt
			...
			console.log("this.insrtPtArray[i] ============= " + this.insrtPtArray[i] );
...
class GameP extends BoxGame{//console.log(" =" + );
	...
	playOver(){
		...
		if(--this.trysLeft == 0){
			...
			let pts = document.getElementById('pts') + 0.0;
			console.log("pts =" + pts);
			pts = this.trysLeft/this.trysPerPlayer;
			document.getElementById('pts').value= pts;
		}
		console.log("GameP playOver() Top this.trysLeft= "  +  this.trysLeft );
	...
	loadButtonValues(){//GameP
		...
		for (let i = 0;i<this.nbrOfButs;i++){ 
         ...
         //console.log("AAthis.butArray[i].value = " + this.butArray[i].value)
...
class ControlPanel{//          console.log(" = " + );
	...
	changePlayers(){
		if(this.playerNbr++ >this.playerArray.length ){
			this.playerNbr = 0;
		}
...
function test(){
	...
	const utl = new Utl();
	//const mm = "planet;sun;mercury;venus;earth;mars;jupiter;saturn;uranus;neptune;pluto";
	const x = document.getElementsByClassName('but').value;

	gameP.helloWorld();
	const v1 = document.getElementById("question").value;
...
function startGame(){//AA.Men00021
	...
	cp.init();
	const p1 = new Player("Pete,16,94066");
	cp.regPlayer(p1);
	const p2 = new Player("Joe,12,91234");
	cp.regPlayer(p2);
...
function setButtonAsSelected(butID){
    console.log("********** setButtonAsSelected "  );
	const but = document.getElementById(butID);//"background-color:aquamarine;";
	console.log("AAsetButtonAsSelected but.disabled= " + but.disabled );
	but.style="background-color:#DC143C;";
	but.disabled = true;
	console.log("BBsetButtonAsSelected but.disabled " + but.disabled )
}

function setButtonAsNotSelected(butID){
	const but = document.getElementById(butID);
	but.disabled = false;
	but.style="background-color:background-color:aquamarine;";
