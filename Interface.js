...
constructor (cp){
    ...
    this.whoHasBid = "";
    this.maxButs = 0;
    this.LogThePlay = false;//This is get around a problem with testing
    }
    ...
    startPlay() {
        ...
        if (bidButs.whoHasBid == "Blue") {//This where I convert from Right Left to 0 and 1
            ...
            //postNotice("BLUE Has The PLAY")
            postNoticeCenterDisplay(16, "Blue", "BLUE Has The PLAY",)
            this.pNbr = 0;//= now PLAYING
            this.theOtherSide = 1;
        } else {
            //postNotice("RED Has The PLAY")
            postNoticeCenterDisplay(16, "Red", "RED Has The PLAY",)
            this.pNbr = 1;
            this.theOtherSide = 0;
        }
    ...
    gameTypeBStartPlay() {
        console.log("^^^gameTypeBStartPlay pNbr= " + this.pNbr)
        this.pointsThisPlay = settings.getPointsForThisPlay(bidButs.topBid);
    ...
    startPlayTimer() {
        ...
        const time = settings.getTime(this.pNbr, bidButs.topBid);
  ...
  updateScoreBoards(){console.log("updateScoreBoards top " )
    ...
    displayPtsThisPlay(passFail, results){console.log("*|*|*displayPts  " + passFail + "   this.nowPlaying= " + this.nowPlaying + "  results = " + results);
        ...
        console.log("this.pointsThisPlay= " + this.pointsThisPlay)//comes from script stopTheClock
        let bonusPts = Math.round(this.pointsThisPlay * this.timeFactor);
        ...
        if(passFail == "passed"){
            ...
            this.displayResultsOfPlay(this.nowPlaying, this.pointsThisPlay, "p", bonusPts);//totalPts includes Bonus
            ...
            this.displayResultsOfPlay(this.theOtherSide,this.pointsThisPlay,"f",bonusPts);//ptsThisPlay does not include Bonus
            this.playAudioThisPlay(this.nowPlaying,false);
        }
        cp.theGameInPlay.reset(passFail);
        if(passFail == "passed"){
            //const remButs = bidButs.remButs;

            //onsole.log("remButs = " + bidButs.remButs);
            if (bidButs.remButs <= 1) {
                postNoticeCenterDisplay(16, "white", "On to the next Round");
                //postNotice(!"On to the next Round");
                this.displayNxtRndBut();
                //this.nextRound();
            }
        }else if(this.failedPlayCount == 0){//Set at 3
            //postNotice(!"On to the next Round");
            postNoticeCenterDisplay(16, "white", "On to the next Round");
            this.displayNxtRndBut();
            //this.nextRound();
        ...
        if (cp.itfType == "B") {
            ...
            console.log(" bidButs.remBut=  " + bidButs.remButs)
        ...
        if (this.LogThePlay)plu.logPlay(this.nowPlaying, passFail, Math.round(this.pointsThisPlay), Math.round(bonusPts));
    }
    ...
    displayResultsOfPlay(player, pts, passfail) {
        ...
        //this.centerDisplay.textContent = winLoseBuf;
        ...
        postNoticeCenterDisplay(16, plu.players[Number(pNbr)].color, winLoseBuf);//
      }
   ...
   registerDocs(){console.log("itf.regDisplayPts " )
       ...
       this.nextRoundButDoc = document.getElementById("nextRoundBut");//centerDisplay"
       ...
       this.ptsThisPlayDoc = document.getElementById("centerDisplay");// changed from "ptsThisPlay"
       this.centerDisplay = document.getElementById("centerDisplay");
  ...
  setPlayerUp(nbr){console.log("&&&itf.setPlayerUp  " + nbr);
    //this.nowPlayingDoc = document.getElementById("nowPlaying");
    this.centerDisplay.textContent = "*NOW PLAYING*"
    if(nbr == 0){
        this.nowPlaying = 0;
        this.theOtherSide =1;
   ...
   timesUp(timeFactor){console.log("itf.timesUp() ");
    ...
    setQuestion(theQuestion) {
        //console.log("setQuestion " + theQuestion);
  ...
  setCheckButXX(){console.log("setCheckBut   " )
   ...
   postNotice(txt){//console.log("postNotice() txt= " + txt + "  this.docsHaveBeenRegistered = " + this.docsHaveBeenRegistered)
       if (this.centerDisplay != null) this.centerDisplay.innerHTML = txt;
   }
    ...
    setNowPlaying(arr) {
        ...
        this.centerDisplay.textContent = arr[2] + "is going first!";
    ...
    setUpPlayArea() {
        console.log("itf setUpPlayArea");
        if (!this.playAreaIsSetUp) {
            this.hideCheckBut();
            this.playAreaIsSetUp = true;
            this.centerDisplay.addEventListener('click', function (event) {
                cp.itf.postNotice("");
            });
        }
    }
    ...
    gameTypeDNoticeXX() {
        console.log("Game.audioToStartGameD");
        const txt = `The Point Factor increases until either player presses the 'Press to Go First' button.
		At which time, it will start to decrease. 
    ...
    makeBidXX(ltrNbr) {
        console.log("makBid " + ltrNbr + " remBut=  " + bidButs.remButs);

        [this.whoHasBid, bidButs.topBid] = ltrNbr.split(":");

        console.log("side= " + this.whoHasBid);
        console.log("bidButs.topBid= " + bidButs.topBid);

        if (bidButs.topBid == bidButs.remButs) {
            startSet();
        }
        let buf = "";
        if (this.whoHasBid == "L") {
            this.whoHasBid = "Blue"
            buf += "Blue bid " + bidButs.topBid;
            this.centerDisplay.style.backgroundColor = 'lightblue';
        } else {
            this.whoHasBid = "Red"
            buf += "Red bid " + bidButs.topBid;
        ...
        let aBut = "";
        let s = ""
        for (let i = 1; i <= bidButs.topBid; i++) {
            s = "bidButL" + i;
            console.log("s= " + s);
            aBut = document.getElementById(s);
            aBut.disabled = true;
